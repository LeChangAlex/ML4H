...
class VAE(Module):
    ...
    """
    def __init__(self, rec_log_prob, prior_network,
    ...
    def batch_vlb(self, batch, output):
        """
        Compute differentiable lower bound for the given batch of objects
        and mask.
        """
        prior = self.make_latent_distributions(batch)
        ...
        rec_loss = self.rec_log_prob(output, rec_params)
        kl = kl_divergence(prior, Normal(0, 1)).view(batch.shape[0], -1).sum(-1)

        # print(output[0, 0, 0], "=========")
        # print(rec_params[0, 0, 0], "------------")
    ...
    def features(self, batch, mask):
        ...
        # torchvision.utils.save_image(rec_params[0, 0].reshape(1, 1, 512, 512), "test/test_.png", normalize=False, nrow=2)

        rec_loss = self.rec_log_prob(batch * mask, rec_params * mask)
        kl = kl_divergence(prior, Normal(0, 1)).view(batch.shape[0], -1).sum(-1)

        return rec_loss, kl, prior_regularization, rec_params
    ...
    def batch_iwae(self, batch, output, K):
        """
        ...
        for i in range(K):
            ...
            estimate = rec_loss + kl
            estimates.append(estimate[:, None])


        return torch.logsumexp(torch.cat(estimates, 1), 1) - math.log(K), rec_params
    ...
    def forward(self, x, output):
        ...
        return self.batch_vlb(x, output)
