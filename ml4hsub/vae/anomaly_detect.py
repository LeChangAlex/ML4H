...
def get_lr_features(query_images, anomaly_score=False, fn=None):


	if fn:
		losses = torch.load(fn)
		print(losses["p_losses"])
		return torch.stack((losses["p_losses"], losses["mse_losses"]), 1)

		latents = torch.load(fn)["latents"].cuda()
	...
	for i in range(len(query_images)):
		...
		with torch.no_grad():
			...
			if fn:
				img_gen = model.generative_network(latents[i:i+1].unsqueeze(2).unsqueeze(2))[:, 0].unsqueeze(1)
				torchvision.utils.save_image(img_gen, "test/test{}_{}.png".format(query_images.intensity, i), normalize=False, nrow=2)

				pixel_losses.append(F.mse_loss(query, img_gen).item())
				latent_losses.append(0)
				reg_losses.append(0)
				...
				rec_loss, kl, prior_regularization, img_gen = model.features(query, mask_ar)
				torchvision.utils.save_image(img_gen[:, 0].unsqueeze(1), "test/test{}_{}.png".format(query_images.intensity, i), normalize=True, nrow=2)
					...
					for mse in [0.5]:
						...
						for run in [9]:
							...
							with torch.no_grad():
								...
								# healthy_features =  get_lr_features(healthy_query, anomaly_score=ascore, fn="latents/test_diseased_0.0_10.0_9_vgg_vae_w_0.5_30_zero_losses")
								# diseased_features = get_lr_features(diseased_query, anomaly_score=ascore, fn="latents/test_diseased_0.4_10.0_9_vgg_vae_w_0.5_30_zero_losses")
